...
| ----------------------------------------------- | --------------------------------------------------------------- |
| `"program": "${workspaceRoot}/dist/server.js",` | Set to our entry point in `dist`                             |
...
### Running tests
To run the tests we again use the script defined in `package.json`:
```
npm test
```
The script on its turn launches `jest`.
